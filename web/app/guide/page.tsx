...
import { PipelineDiagram } from "../../components/PipelineStepper";
...
export default function GuidePage() {
  return (
    <div style={{ maxWidth: 860 }}>
      ...
      <p style={{ opacity: 0.6, fontSize: 13, margin: "0 0 20px" }}>
      ...
      {/* Pipeline diagram */}
      <h2 style={{ fontSize: 16, margin: "22px 0 10px" }}>Pipeline — idea → reflected</h2>
      <PipelineDiagram />
      ...
      {/* Inline SVG pipeline */}
      <div style={{ marginTop: 12, background: "#0f1620", border: "1px solid #1e2f44", borderRadius: 14, padding: 16, overflowX: "auto" }}>
        <svg viewBox="0 0 760 64" width="760" height="64" style={{ display: "block", maxWidth: "100%" }}>
          {[
            ["Idea", "#1a2740"],
            ["Brief", "#1a2740"],
            ["Draft", "#2a220a"],
            ...
            return (
              <g key={label}>
                <rect x={x} y={8} rx={10} ry={10} width={78} height={28} fill={fill} stroke="#2a3a52" />
                <text x={x + 39} y={26} textAnchor="middle" fontSize="10" fontWeight="700" fill="#cfe0ff">{label}</text>
                {i < 7 && <text x={x + 80} y={26} fontSize="12" fill="#2a3a52">→</text>}
              </g>
            );
          })}
          <text x={12} y={54} fontSize="9" fill="#5a6b86">human seeds → AI drafts → human gate → system delivers → observe → learn</text>
        </svg>
      </div>

      {/* Statuses table */}
      <h2 style={{ fontSize: 16, margin: "24px 0 10px" }}>Statuses</h2>
      <div style={{ background: "#0f1620", border: "1px solid #1e2f44", borderRadius: 12, overflow: "hidden" }}>
          ...
          <tbody>
            {STATUSES.map((r) => (
              <tr key={r.status} style={{ borderBottom: "1px solid #1e2f44" }}>
                <td style={{ padding: "8px 10px" }}><StatusBadge status={r.status} size={11} /></td>
      ...
      {/* Queue / Projects / Channels */}
      <h2 style={{ fontSize: 16, margin: "24px 0 10px" }}>Queue / Projects / Channels</h2>
      <div style={{ display: "grid", gap: 10 }}>
        <div style={{ background: "#0f1620", border: "1px solid #1e2f44", borderRadius: 12, padding: 14 }}>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "#8FA0B8" }}>The review queue: all items for the signed-in owner, newest first. Click a title to open <code style={{ background: "#0b1420", padding: "1px 6px", borderRadius: 6 }}>/items/[id]</code> — stepper shows progress, badge shows status, channels selector picks delivery.</p>
        </div>
        <div style={{ background: "#0f1620", border: "1px solid #1e2f44", borderRadius: 12, padding: 14 }}>
          <strong style={{ fontSize: 13, color: "#cfe0ff" }}>Projects</strong> <span style={{ fontSize: 11, color: "#5a6b86" }}>— /projects</span>
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "#8FA0B8" }}>A project owns editorial policy (languages, channels list, policy JSON). Every content item belongs to one project. Opening <code style={{ background: "#0b1420", padding: "1px 6px", borderRadius: 6 }}>/projects/[id]</code> lets you manage its channels. Required when creating an item.</p>
        </div>
        <div style={{ background: "#0f1620", border: "1px solid #1e2f44", borderRadius: 12, padding: 14 }}>
          <strong style={{ fontSize: 13, color: "#cfe0ff" }}>Channels</strong> <span style={{ fontSize: 11, color: "#5a6b86" }}>— /settings/channels</span>
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "#8FA0B8" }}>Per-user, per-project channel bindings. Each channel has a <code>type</code> (e.g. <code>telegram</code>, <code>familyos_pattayadom</code>), encrypted config, and a connectivity <code>status</code>. Use “Check” to verify. Channels are selected at the item level before publish; Channel-state is independent of the item status.</p>
        </div>
      </div>
...
// decrypts Channel.config_encrypted → selects adapter by Channel.type`}</pre>
        <ul style={{ margin: "10px 0 0", paddingLeft: 16, fontSize: 12, color: "#8FA0B8", lineHeight: 1.7 }}>
          <li><code>telegram</code> — Telegram Bot API (existing adapter in <code style={{ background: "#0b1420", padding: "1px 6px", borderRadius: 6 }}>internal/adapters/telegram</code>)</li>
          <li><code>familyos_pattayadom</code> — FamilyOS PattayaDom (existing in <code style={{ background: "#0b1420", padding: "1px 6px", borderRadius: 6 }}>internal/adapters/familyos</code>)</li>
          <li>New type → implement <code>Publisher</code>, register in <code>adapters/factory.go:PublisherFromChannel</code>, add capability flags.</li>
          <li>Idempotency: <code>createDraft</code> takes <code>idempotencyKey</code> (publication.idempotency_key) to prevent double-post.</li>
        </ul>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        {[
          { n: "1", title: "Create project + channels", body: <>Go to <Link href="/projects" style={{ color: "#8fb8ff" }}>Projects</Link> → New project, then <Link href="/settings/channels" style={{ color: "#8fb8ff" }}>Channels</Link> → Add Telegram / FamilyOS. “Check” until status is OK.</> },
          { n: "2", title: "Create item & generate", body: <>Queue → <strong>+ New item</strong> → title + brief + project → Create. Brief is auto-scaffolded; open the item and click <strong>Generate</strong> to draft. Iterate via Review.</> },
          { n: "3", title: "Approve & publish", body: <>In the item page select channels, click <strong>Approve</strong> then <strong>Publish</strong>. Per-channel delivery appears; use <strong>Fetch metrics</strong> later to move to <em>measuring</em> → <em>reflected</em>.</> },
        ].map((s) => (
      ...
      <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
        <Link href="/" style={{ background: "linear-gradient(135deg,#3D8DFF,#6DCBF4)", color: "#fff", borderRadius: 10, padding: "9px 16px", textDecoration: "none", fontWeight: 700, fontSize: 13 }}>Go to Queue →</Link>
        <Link href="/projects" style={{ background: "#1a2636", border: "1px solid #2a3a52", color: "#8fb8ff", borderRadius: 10, padding: "9px 16px", textDecoration: "none", fontSize: 13 }}>Projects</Link>
        <Link href="/settings/channels" style={{ background: "#1a2636", border: "1px solid #2a3a52", color: "#8fb8ff", borderRadius: 10, padding: "9px 16px", textDecoration: "none", fontSize: 13 }}>Channels</Link>
      </div>
